...
export const seasons = pgTable("seasons", {
  ...
  name: varchar("name", { length: 100 }).notNull(),
  status: varchar("status", { length: 20 }).notNull().default("upcoming"), // "active" | "archived" | "upcoming"
...
// Week configs table — custom labels for each week in a season
export const weekConfigs = pgTable("week_configs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const insertWeekConfigSchema = createInsertSchema(weekConfigs).omit({
  id: true,
});

export type InsertWeekConfig = z.infer<typeof insertWeekConfigSchema>;

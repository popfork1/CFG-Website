...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Default logo map used for seeding and backfilling
  const DEFAULT_TEAM_LOGOS: Record<string, string> = {
  ...
  app.get("/api/teams", async (req, res) => {
    try {
      let allTeams = await storage.getAllTeams();
      if (allTeams.length === 0) {
        // Seed teams with logos
        const defaultTeams = [
          { name: "Green Bay Packers", city: "Green Bay", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Green Bay Packers"] },
          { name: "New York Cobras", city: "New York", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["New York Cobras"] },
          { name: "Chicago Grizzlies", city: "Chicago", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["Chicago Grizzlies"] },
          { name: "Pittsburgh Steelers", city: "Pittsburgh", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["Pittsburgh Steelers"] },
          { name: "Baltimore Ravens", city: "Baltimore", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["Baltimore Ravens"] },
          { name: "Miami Gators", city: "Miami", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Miami Gators"] },
          { name: "Denver Broncos", city: "Denver", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Denver Broncos"] },
          { name: "Tennessee Titans", city: "Tennessee", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Tennessee Titans"] },
          { name: "San Francisco 49ers", city: "San Francisco", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["San Francisco 49ers"] },
          { name: "Tampa Bay Buccaneers", city: "Tampa Bay", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["Tampa Bay Buccaneers"] },
          { name: "Dallas Cowboys", city: "Dallas", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Dallas Cowboys"] },
          { name: "Minnesota Vikings", city: "Minnesota", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Minnesota Vikings"] },
          { name: "Jacksonville Ironbacks", city: "Jacksonville", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["Jacksonville Ironbacks"] },
          { name: "Indianapolis Colts", city: "Indianapolis", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Indianapolis Colts"] },
          { name: "Cleveland Browns", city: "Cleveland", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["Cleveland Browns"] },
          { name: "Philadelphia Eagles", city: "Philadelphia", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Philadelphia Eagles"] },
          { name: "Los Angeles Rams", city: "Los Angeles", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["Los Angeles Rams"] },
          { name: "Carolina Panthers", city: "Carolina", division: "NFC_East", logo: DEFAULT_TEAM_LOGOS["Carolina Panthers"] },
          { name: "San Antonio Brahmas", city: "San Antonio", division: "NFC_West", logo: DEFAULT_TEAM_LOGOS["San Antonio Brahmas"] },
          { name: "Buffalo Bills", city: "Buffalo", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["Buffalo Bills"] },
          { name: "New England Patriots", city: "New England", division: "AFC_East", logo: DEFAULT_TEAM_LOGOS["New England Patriots"] },
          { name: "Nevada Nighthawks", city: "Nevada", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Nevada Nighthawks"] },
          { name: "Houston Texans", city: "Houston", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Houston Texans"] },
          { name: "Las Vegas Raiders", city: "Las Vegas", division: "AFC_West", logo: DEFAULT_TEAM_LOGOS["Las Vegas Raiders"] },
        ];
        for (const team of defaultTeams) {
          await storage.createTeam(team);
        }
        allTeams = await storage.getAllTeams();
      } else {
        // Backfill logos for existing teams that have no logo set
        const teamsNeedingLogos = allTeams.filter(t => !t.logo && DEFAULT_TEAM_LOGOS[t.name]);
        for (const team of teamsNeedingLogos) {
          await storage.updateTeam(team.id, { logo: DEFAULT_TEAM_LOGOS[team.name] });
        }
  ...
  async function ensureSeasonsSeeded() {
    const existing = await storage.getAllSeasons();
    if (existing.length > 0) return;
    const allGames = await storage.getAllGames();
    const seasonNums = [...new Set(allGames.map(g => g.season ?? 1))].sort((a, b) => a - b);
    if (seasonNums.length > 0) {
      ...
      for (const num of seasonNums) {
        await storage.createSeason({
          number: num,
          name: `Season ${num}`,
          status: num === maxSeason ? "active" : "archived",
          totalWeeks: num === 1 ? 14 : 15,
          year: 2024 + (num - 1),
        });
      }
    } else {
      await storage.createSeason({ number: 1, name: "Season 1", status: "archived", totalWeeks: 14, year: 2024 });
      await storage.createSeason({ number: 2, name: "Season 2", status: "active", totalWeeks: 15, year: 2025 });
    }
  }
  ...
  app.get("/api/seasons/active", async (_req, res) => {
    try {
      ...
      const weekCfgs = await storage.getWeekConfigs(active.id);
      res.json({ ...active, weekConfigs: weekCfgs });
    } catch (error) {
  ...
  app.post("/api/seasons", isAuthenticated, async (req, res) => {
    if (req.session?.role !== "admin") return res.status(403).json({ message: "Admin only" });
    try {
      const { number, name, status, totalWeeks, year } = req.body;
      if (!number || !name) return res.status(400).json({ message: "number and name required" });
      const season = await storage.createSeason({ number: Number(number), name, status: status || "upcoming", totalWeeks: Number(totalWeeks) || 15, year: year ? Number(year) : null });
      res.json(season);
    } catch (error: any) {
      console.error("Error creating season:", error);
      res.status(500).json({ message: error?.message || "Failed to create season" });
    }
  });
  ...
  app.patch("/api/seasons/:id", isAuthenticated, async (req, res) => {
    if (req.session?.role !== "admin") return res.status(403).json({ message: "Admin only" });
    try {
      const { name, totalWeeks, year } = req.body;
      const updated = await storage.updateSeason(req.params.id, {
        ...(name !== undefined && { name }),
  ...
  app.patch("/api/seasons/:id/activate", isAuthenticated, async (req, res) => {
    if (req.session?.role !== "admin") return res.status(403).json({ message: "Admin only" });
    try {
      const season = await storage.activateSeason(req.params.id);
      res.json(season);
    } catch (error) {
  ...
  app.patch("/api/seasons/:id/archive", isAuthenticated, async (req, res) => {
    if (req.session?.role !== "admin") return res.status(403).json({ message: "Admin only" });
    try {
      const season = await storage.archiveSeason(req.params.id);
      res.json(season);
    } catch (error) {
  ...
  app.get("/api/seasons/:id/weeks", async (req, res) => {
    try {
      const cfgs = await storage.getWeekConfigs(req.params.id);
      res.json(cfgs);
    } catch (error) {
      console.error("Error fetching week configs:", error);
      res.status(500).json({ message: "Failed to fetch week configs" });
    }
  });

  app.put("/api/seasons/:id/weeks", isAuthenticated, async (req, res) => {
    if (req.session?.role !== "admin") return res.status(403).json({ message: "Admin only" });
    try {
      const { configs } = req.body;
      if (!Array.isArray(configs)) return res.status(400).json({ message: "configs must be an array" });
      const saved = await storage.setWeekConfigs(req.params.id, configs);
      res.json(saved);
    } catch (error) {
      console.error("Error saving week configs:", error);
      res.status(500).json({ message: "Failed to save week configs" });
    }
  });

...
export interface IStorage {
  ...
  getSeasonByNumber(number: number): Promise<Season | undefined>;
  createSeason(season: InsertSeason): Promise<Season>;
  updateSeason(id: string, data: Partial<InsertSeason>): Promise<Season>;
  deleteSeason(id: string): Promise<void>;
  activateSeason(id: string): Promise<Season>;
  archiveSeason(id: string): Promise<Season>;

  // Week config management
  getWeekConfigs(seasonId: string): Promise<WeekConfig[]>;
  setWeekConfigs(seasonId: string, configs: { weekNumber: number; label: string; isPlayoff: boolean }[]): Promise<WeekConfig[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getActiveSeason(): Promise<Season | undefined> {
    ...
    // Fall back to highest-numbered season
  ...
  async getSeasonByNumber(number: number): Promise<Season | undefined> {
    const [season] = await db.select().from(seasons).where(eq(seasons.number, number)).limit(1);
    return season;
  }
  ...
  async updateSeason(id: string, data: Partial<InsertSeason>): Promise<Season> {
  ...
  async activateSeason(id: string): Promise<Season> {
    // Set all to upcoming/archived first, then activate the target
    await db.update(seasons).set({ status: "upcoming", updatedAt: new Date() }).where(eq(seasons.status, "active"));
    const [updated] = await db.update(seasons).set({ status: "active", updatedAt: new Date() }).where(eq(seasons.id, id)).returning();
    return updated;
  }
  ...
  // Week config management
  async getWeekConfigs(seasonId: string): Promise<WeekConfig[]> {
    return db.select().from(weekConfigs).where(eq(weekConfigs.seasonId, seasonId));
  }

  async setWeekConfigs(seasonId: string, configs: { weekNumber: number; label: string; isPlayoff: boolean }[]): Promise<WeekConfig[]> {
    await db.delete(weekConfigs).where(eq(weekConfigs.seasonId, seasonId));
    if (configs.length === 0) return [];
    const inserted = await db.insert(weekConfigs).values(
      configs.map(c => ({ seasonId, weekNumber: c.weekNumber, label: c.label, isPlayoff: c.isPlayoff ?? false }))
    ).returning();
    return inserted;
  }
}

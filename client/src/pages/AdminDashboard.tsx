          ...
          <Card className="p-8 md:p-12 bg-card/40 backdrop-blur-3xl border-border/40 rounded-[48px] shadow-2xl relative overflow-hidden">
            ...
            {role === "admin" && (
              <>
                ...
                <TabsContent value="standings" className="mt-0 outline-none"><StandingsManager /></TabsContent>
...
function TeamsManager() {
  ...
  const { data: divisionsList = [] } = useQuery<any[]>({ queryKey: ["/api/divisions"] });
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Select value={editDivision} onValueChange={setEditDivision}>
                      <SelectTrigger data-testid={`select-division-${team.id}`}>
                        <SelectValue placeholder="No division assigned" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none">No division</SelectItem>
                        {divisionsList.map((div: any) => (
                          <SelectItem key={div.id} value={div.key}>
                            {div.conference} — {div.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      onClick={() => updateMutation.mutate({ id: team.id, data: { name: editName, logo: editLogo || null, division: editDivision === "none" ? null : editDivision || null } as any })}
...
function StandingsManager() {
  ...
  const { data: standingsList = [], isLoading } = useQuery<any[]>({ queryKey: ["/api/standings"] });
  const { data: activeSeason } = useActiveSeason();
  const [edits, setEdits] = useState<Record<string, { wins: number; losses: number; pd: number }>>({});

  const getStanding = (teamName: string) =>
    standingsList.find((s: any) => s.team?.toLowerCase() === teamName.toLowerCase()) ?? null;

  const getVals = (teamName: string) => {
    if (edits[teamName] !== undefined) return edits[teamName];
    const s = getStanding(teamName);
    return { wins: s?.wins ?? 0, losses: s?.losses ?? 0, pd: s?.pointDifferential ?? 0 };
  };

  const setField = (teamName: string, field: "wins" | "losses" | "pd", val: number) =>
    setEdits(prev => ({ ...prev, [teamName]: { ...getVals(teamName), [field]: val } }));

  const saveMutation = useMutation({
    mutationFn: async ({ teamName, divKey }: { teamName: string; divKey: string }) => {
      const vals = getVals(teamName);
      const existing = getStanding(teamName);
      const season = activeSeason?.number ?? 1;
      if (existing) {
        await apiRequest("PATCH", `/api/standings/${existing.id}`, {
          wins: vals.wins, losses: vals.losses, pointDifferential: vals.pd,
        });
      } else {
        await apiRequest("POST", "/api/standings", {
          team: teamName, division: divKey, season,
          wins: vals.wins, losses: vals.losses, pointDifferential: vals.pd,
        });
      }
    },
    onSuccess: (_data, vars) => {
      ...
      setEdits(prev => { const n = { ...prev }; delete n[vars.teamName]; return n; });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/standings/${id}`, undefined),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/standings"] });
      toast({ title: "Standing reset" });
    },
    onError: () => toast({ title: "Error", description: "Failed to reset", variant: "destructive" }),
  });

  if (isLoading) return <div className="text-center py-16 text-muted-foreground text-xs font-black uppercase tracking-widest">Loading...</div>;

  return (
    <div className="space-y-10">
      <div className="flex items-center gap-3">
        <div className="w-1.5 h-6 bg-primary rounded-full" />
        <h2 className="text-2xl font-black italic uppercase tracking-tight">Manage Standings</h2>
      </div>
      <p className="text-sm text-muted-foreground -mt-6">Edit wins, losses, and point differential per team. Changes show instantly on the Teams page.</p>

      {divisions.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground/30 text-xs font-black uppercase tracking-widest">No divisions configured — add divisions first</div>
      ) : (
        <div className="space-y-8">
          {divisions.map((div: any) => {
            const divTeams = (teams as any[]).filter((t: any) => t.division === div.key);
            return (
              <div key={div.id} className="space-y-3">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: div.conferenceColor ?? "#3b82f6" }} />
                  <h3 className="text-sm font-black uppercase tracking-widest" style={{ color: div.conferenceColor ?? "#3b82f6" }}>
                    {div.conference} — {div.label}
                  </h3>
                </div>
                {divTeams.length === 0 ? (
                  <p className="text-xs text-muted-foreground/40 pl-4">No teams assigned to this division</p>
                ) : (
                  <div className="space-y-2">
                    {divTeams.map((team: any) => {
                      const vals = getVals(team.name);
                      const existing = getStanding(team.name);
                      const isDirty = edits[team.name] !== undefined;
                      return (
                        <Card key={team.id} className="p-4 bg-white/5 border-border/30 rounded-2xl">
                          <div className="flex items-center gap-4 flex-wrap">
                            {team.logo && <img src={team.logo} alt={team.name} className="w-8 h-8 object-contain flex-shrink-0" />}
                            <span className="font-black text-sm uppercase tracking-tight flex-1 min-w-[120px]">{team.name}</span>
                            <div className="flex items-center gap-3 flex-wrap">
                              {(["wins", "losses", "pd"] as const).map((field) => (
                                <div key={field} className="flex items-center gap-1.5">
                                  <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                                    {field === "pd" ? "PD" : field === "wins" ? "W" : "L"}
                                  </label>
                                  <input
                                    type="number"
                                    className="w-14 h-8 px-2 rounded-lg bg-white/5 border border-white/10 text-sm font-black text-center focus:outline-none focus:ring-2 focus:ring-primary/30"
                                    value={vals[field]}
                                    onChange={e => setField(team.name, field, parseInt(e.target.value) || 0)}
                                    data-testid={`input-${field}-${team.id}`}
                                  />
                                </div>
                              ))}
                              <Button
                                ...
                                onClick={() => saveMutation.mutate({ teamName: team.name, divKey: div.key })}
                                className="rounded-lg font-black text-[10px] uppercase h-8 px-4"
                                data-testid={`button-save-standing-${team.id}`}
                              >
                                Save
                              </Button>
                              {existing && (
                                <Button
                                  ...
                                  className="rounded-lg font-black text-[10px] uppercase h-8 px-3"
                                  data-testid={`button-reset-standing-${team.id}`}
                                >
                                  Reset
                                </Button>
                              )}

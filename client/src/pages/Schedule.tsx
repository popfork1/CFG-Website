...
import { useActiveSeason, getWeekLabel } from "@/hooks/useSeasons";
...
export default function Schedule() {
  ...
  const seasonNumber = activeSeason?.number ?? 2;
  ...
  const filteredGames = allGames ? allGames.filter(game => {
    return game.team1.toLowerCase().includes(searchQuery.toLowerCase()) ||
            ...
            <div key={week} className="space-y-8">
              ...
              <div className="grid gap-4">
                {gamesByWeek[week].map((game) => {
                  ...
                  const logo1 = getLogoUrl(game.team1);
                  const logo2 = getLogoUrl(game.team2);
                  
                  return (
                    <Card key={game.id} className="group p-6 bg-card/40 backdrop-blur-xl border-border/40 hover:bg-card/60 transition-all duration-300 rounded-[32px] overflow-hidden" data-testid={`card-schedule-game-${game.id}`}>
                        ...
                        <div className="flex items-center gap-8 flex-1">
                          <div className="flex items-center -space-x-4">
                            <div className="w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center p-2.5 shadow-2xl group-hover:scale-110 transition-transform">
                              {logo2 ? <img src={logo2} className="w-full h-full object-contain" alt={game.team2} /> : <Trophy className="w-6 h-6 text-muted-foreground/20" />}
                            </div>
                            <div className="w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center p-2.5 shadow-2xl group-hover:scale-110 transition-transform">
                              {logo1 ? <img src={logo1} className="w-full h-full object-contain" alt={game.team1} /> : <Trophy className="w-6 h-6 text-muted-foreground/20" />}
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-8 lg:text-right border-t lg:border-t-0 border-white/5 pt-6 lg:pt-0">
                          ...
                          {game.isFinal && (
                            <div className="text-right">
                              ...
                              <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Final</div>
                            </div>
                          )}
                        </div>
                      </div>
